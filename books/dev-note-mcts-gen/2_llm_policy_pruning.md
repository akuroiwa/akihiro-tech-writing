...
## GPからPrompt-MCTSへの進化：条件コードから自然言語プロンプトへ（2026年8月）

2026年8月の開発において、`mcts-gen` はさらに重要な質的転換を遂げました。それは、探索木を誘導するロジックの表現形式を「遺伝的プログラミング (GP) 由来の `if_then_else` 条件コードの生成」から、「**ドメイン固有の自然言語プロンプト（Prompt-MCTS）**」へと移行したことです。
...
人間が推論する際と同じように、条件分岐コードを静的に生成させるのではなく、`src/mcts_gen/games/*_mcts.py`（例: チェスや将棋における「王手になる手を優先する」、創薬における化学的ヒューリスティクス）の中に**自然言語によるドメインヒューリスティクス（プロンプト）**を定義するアプローチを採用しました。

この「Prompt-MCTS」により、LLMの表現力と推論力を損なうことなく、探索木の枝切りと評価をきわめて自然に導くことが可能になったのです。
