...
チェスの「駒の動きを選択して王手を目指す」プロセスは、化学における「定義されたフラグメント（分子断片）を選択・結合させてターゲット分子に類似した分子を生成する」プロセスと、木探索という観点で共通しています。当時の `chem-ant` のアプローチは、あくまで既存のターゲット分子と類似する構造を生成する試みであり、真の意味で「原子や官能基を自由に結合させて目的の物性・性質を持つ分子を一から設計（デザイン）する」プロセスへと至るのは、後の `mcts-gen` の登場を待つことになります。
