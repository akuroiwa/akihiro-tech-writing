...
この5年間の開発を通じて得られた最も強力な知見は、**「どれほど AI 技術が進化し、自律的にコードを書くようになっても、開発者本人がアルゴリズムの根底や泥臭いエラーの実態を100%理解していなければ、AI との真の協業は成り立たない」** という確信です。

第5章や第7章で触れたように、AI（ChatGPT や Bard）が提示する並列処理や非同期通信のコードには、無数の「動かない提案」や「隠れたレースコンディション（デッドロックの罠）」が含まれていました。
...
自分の頭で考え、手で実装し、失敗し続けた「パスタコミットの日々」があったからこそ、AI の提案の善し悪しを即座に見極め、正しい設計方針へと AI を軌道修正し、最高の相棒として使いこなすことができたのです。
